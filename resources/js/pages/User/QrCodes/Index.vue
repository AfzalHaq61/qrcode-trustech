                                ...
                                <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
                                    ...
                                    <tbody>
                                        <tr v-for="(qr_code, index) in qr_codes.data" :key="qr_code.id">
                                            ...
                                            <td
                                                class="px-3 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                <p class="mb-0 text-xs font-semibold leading-tight text-blue-600 dark:text-blue-500  hover:opacity-50"><Link :href="route('user.download.qrcode', { id: qr_code.qr_code_id })">{{ qr_code.name }}</Link></p>
                                            </td>
                                            <td
                                                class="px-3 text-sm leading-normal align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                <!-- <span class="bg-black px-2.5 text-xs rounded-1.8 py-1.4 inline-block whitespace-nowrap text-center align-baseline font-bold uppercase leading-none text-white"><Link :href="route('user.type.qrcodes', { type: qr_code.type })">{{ qr_code.type }}</Link></span> -->
                                                <span class="inline-block rounded-1.8 bg-info px-2.5 py-1.4 text-xs font-bold uppercase whitespace-nowrap leading-normal text-white shadow-[0_4px_9px_-4px_#54b4d3] transition duration-150 ease-in-out hover:bg-info-600 hover:shadow-[0_8px_9px_-4px_rgba(84,180,211,0.3),0_4px_18px_0_rgba(84,180,211,0.2)] focus:bg-info-600 focus:shadow-[0_8px_9px_-4px_rgba(84,180,211,0.3),0_4px_18px_0_rgba(84,180,211,0.2)] focus:outline-none focus:ring-0 active:bg-info-700 active:shadow-[0_8px_9px_-4px_rgba(84,180,211,0.3),0_4px_18px_0_rgba(84,180,211,0.2)] dark:shadow-[0_4px_9px_-4px_rgba(84,180,211,0.5)] dark:hover:shadow-[0_8px_9px_-4px_rgba(84,180,211,0.2),0_4px_18px_0_rgba(84,180,211,0.1)] dark:focus:shadow-[0_8px_9px_-4px_rgba(84,180,211,0.2),0_4px_18px_0_rgba(84,180,211,0.1)] dark:active:shadow-[0_8px_9px_-4px_rgba(84,180,211,0.2),0_4px_18px_0_rgba(84,180,211,0.1)]"><Link :href="route('user.type.qrcodes', { type: qr_code.type })">{{ qr_code.type }}</Link></span>
                                            </td>
                                            <td
                                                class="px-3 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                                                    ...
                                                    <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                                                        <MenuItems class="absolute right-0 z-100 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                                            ...
                                                            <MenuItem v-slot="{ active }">
                                                                <Link :href="route('user.qr.statistics', { id: qr_code.qr_code_id })" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-2 text-xs font-semibold leading-tight text-slate-400']">Statistics</Link>
                                                            </MenuItem>
                                                            <MenuItem v-slot="{ active }">
                                                                <Link :href="route('user.download.qrcode', { id: qr_code.qr_code_id })" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-2 text-xs font-semibold leading-tight text-slate-400']">Download QR</Link>
                                                            </MenuItem>
                                                            <MenuItem v-slot="{ active }">
                                                                <Link :href="route('user.edit.qr', { id: qr_code.qr_code_id })" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-2 text-xs font-semibold leading-tight text-slate-400']">Edit</Link>
                                                            </MenuItem>
                                                            <MenuItem v-slot="{ active }" @click="open = true">
                                                                <Link :href="route('user.update.qr.status', { id: qr_code.qr_code_id })" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-2 text-xs font-semibold leading-tight text-slate-400']"><span v-if="qr_code.status === 0">Activate</span><span v-else>Deactivate</span></Link>
                                                            </MenuItem>
                                                            <MenuItem v-slot="{ active }">
                                                                <Link :href="route('user.delete.qr', { id: qr_code.qr_code_id })" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-2 text-xs font-semibold leading-tight text-slate-400']">Delete</Link>
                                                            </MenuItem>
                                                            </div>
                                                        </MenuItems>
                                                    </transition>
